 Shader "Custom/spheres" {
  Properties {
  

    _NumberSteps( "Number Steps", Int ) = 100
    _MaxTraceDistance( "Max Trace Distance" , Float ) = 60.0
  ...
  SubShader {
    ...
    Pass {
      ...
      float2 map( in float3 pos ){
        ...
        res = float2( sdBox( ( pos % float3( 1. , 1. , 1. ))-float3( .5 , .5 , .5 ) , float3( .1 , .1 , .1 ) ) , 0. );

        float2 sphere = float2( sdSphere( pos - _Hand1 , .1 ) , 1. );
        res = smoothU( res , sphere , .1);

        sphere = float2( sdSphere( pos - _Hand2 , .1 ) , 1. );
        res = smoothU( res , sphere , .1);
